...
export function recallTool(memory: MemoryService) {
  return defineTool({
    name: 'memory_recall',
...
export function rememberTool(memory: MemoryService) {
  return defineTool({
...
export function forgetTool(memory: MemoryService) {
  return defineTool({
    name: 'memory_forget',
    ...
    async execute(args) {
      const ok = await memory.forget(args.id, (args.mode as 'delete' | 'archive') ?? 'delete')
      return { ok }
    },
  })
}
...
export function readUserProfileTool(_memory: MemoryService) {
  return defineTool({
