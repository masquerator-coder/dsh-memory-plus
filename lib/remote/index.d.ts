 ...
 *  }
 */
export declare abstract class MemoryRemoteContract {
    abstract stats(): Promise<MemoryStats>;
    abstract soulGet(): Promise<{
        content: string;
        tokenCount: number;
    }>;
    abstract soulPut(patch: {
        content: string;
    }): Promise<{
        ok: boolean;
    }>;
    abstract factsQuery(filter: unknown): Promise<{
        items: FactDetail[];
        total: number;
    }>;
    abstract factUpdate(id: string, patch: unknown): Promise<{
        ok: boolean;
    }>;
    abstract factDelete(id: string): Promise<{
        ok: boolean;
    }>;
