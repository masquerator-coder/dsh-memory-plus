...
const QUALIFIER_KEYS = ['time', 'location', 'context', 'condition'];
/** 从 qualifiers 中提取参与语义键的规范化签名。 */
export function qualifierSignature(qualifiers) {
    if (!qualifiers)
        return '';
    const picked = {};
    for (const key of QUALIFIER_KEYS) {
        const v = qualifiers[key];
...
export function computeSemanticKey(fact) {
    return fnv1a(`${fact.subject.id}|${fact.predicate}|${fact.object.id}|${qualifierSignature(fact.qualifiers)}`);
}
/**
 ...
 */
export function isSameAssertion(a, b) {
    return computeSemanticKey(a) === computeSemanticKey(b);
}
//# sourceMappingURL=semantic-key.js.map