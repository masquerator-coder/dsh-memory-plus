...
function assemble(ctx: Context): MemoryService {
  ...
  const extractor = { extract: (content, scope) => new LlmExtractor(ctx, scope).extract(content) }
  return new MemoryService({ store, vector, extractor })
}

/** 抽取器：经 ctx.llm.stream + BlockAssembler 调用独立 LLM（主会话 LLM 不做抽取）。 */
class LlmExtractor {
  constructor(private ctx: Context, private scope: string) {}
...
export function apply(ctx: Context): void {
  ...
  ctx.effect(() => {
    ...
    const disposers = [
      ...
      ctx.systemPrompt.section({
        ...
        order: ctx.systemPrompt.getSectionOrder?.('THIRD_PARTY') ?? 20000,
        text: [
  ...
  ctx.on('agent/pre-step', async ({ agent, messages, signal }, next): Promise<PreStepDecision> => {
    ...
    const memory = ctx.get<MemoryService>('memory')
  ...
  // 7) 后台定时整合与遗忘
  ctx.timer.interval('dsh-memory-consolidate', () => {
    const memory = ctx.get<MemoryService>('memory')
    if (memory) runConsolidate(ctx, memory)
  }, 6 * 60 * 60 * 1000) // 每 6 小时
