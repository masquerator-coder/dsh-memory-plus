...
export type ValidationResult =
  | { ok: true; fact: AtomicFactInput; warnings: string[] }
  | { ok: false; errors: string[]; fact?: AtomicFactInput }

const MULTI_ENTITY_RE = /[、，,和及以及与]/g

/** 校验单条待入库事实。 */
export function validateFact(input: AtomicFactInput): ValidationResult {
  const errors: string[] = []
  const warnings: string[] = []

  // 1. 三元组完整性
  const s = input.subject
  if (!s || typeof s.type !== 'string' || s.type.length === 0) errors.push('subject.type 缺失')
  if (!s || typeof s.id !== 'string' || s.id.length === 0) errors.push('subject.id 缺失')
  if (typeof input.predicate !== 'string' || input.predicate.length === 0) errors.push('predicate 缺失')
  const o = input.object
  if (!o || typeof o.type !== 'string' || o.type.length === 0) errors.push('object.type 缺失')
  if (!o || typeof o.id !== 'string' || o.id.length === 0) errors.push('object.id 缺失')
  ...
  if (errors.length > 0) return { ok: false, errors, fact: input }
...
export interface SelfContainmentChecker {
  (content: string): Promise<{ selfContained: boolean; reason?: string }>
}
 ...
 */
export async function validateWithChecks(
...
export function dedupeCheck(
  ...
  if (existingKeys.has(key)) return { duplicate: true, existingKey: key }
 ...
 */
export function independentUpdateCheck(
  target: { semantic_key: string },
  all: Array<{ semantic_key: string }>,
): number {
  return all.filter((f) => f.semantic_key === target.semantic_key).length
}

/** 独立遗忘测试：删除该事实是否精确无副作用（只匹配目标 id 一条）。 */
export function independentForgetCheck(
  target: { id: string },
  all: Array<{ id: string }>,
): number {
