...
import { readMemorySettings, MEMORY_SETTINGS_NAMESPACE, MemorySettingsSchema, DEFAULT_EXTRACTION_PROVIDER, DEFAULT_EXTRACTION_MODEL, } from './settings.js';
...
function assemble(ctx) {
    ...
    const extractor = new LlmExtractor(ctx);
 ...
 */
function resolveExtractionRoute(ctx, settings) {
...
const extractionQueue = new Map();
/** 由 sessionId 派生业务 scope（与 `scopeOf`/`user:<id>` 的风格保持一致）。 */
function scopeFromSession(sessionId) {
...
function enqueueExtraction(ctx, sessionId, event) {
 ...
 */
async function drainExtractionQueue(ctx, sessionId) {
    const scope = scopeFromSession(sessionId);
...
async function runConsolidate(ctx, _memory) {
    const logger = ctx.logger('dsh-memory');
    try {
