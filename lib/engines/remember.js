...
import { validateWithChecks } from '../model/validator.js';
/**
 * 记住一批事实（引擎主入口）。对每条候选：
 *  1. 验证（三元组完整 / 粒度 / 自包含）
 ...
 *  3. 写入存储 + 索引向量
 */
export async function rememberMany(candidates, opts) {
    const created = [];
    const updates = [];
    for (const candidate of candidates) {
        ...
        if (opts.vector && created.length > 0) {
            for (const f of created)
                await opts.vector.index(f);
        }
    }
    for (const f of created)
        await opts.onRemember?.(f);
    return { created, updates };
}
/** 构造一条完整事实（填派生字段）。 */
export function buildFact(input, meta) {
...
function resolveConflict(input, existing, strategy) {
    switch (strategy) {
        case 'latest_wins':
